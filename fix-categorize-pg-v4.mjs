...
function guessCategory(text) {
    ...
    for (const cat of CATEGORIES) {
        for (const kw of cat.keywords) {
            // Strict check for short words or common partial matches
            if (['home', 'pan', 'pot', 'pen', 'top', 'bag', 'fan', 'cap'].includes(kw)) {
...
async function main() {
    ...
    try {
        console.log("Fetching products to refine categories (V4 - Fix 'Pan' issue)...");
