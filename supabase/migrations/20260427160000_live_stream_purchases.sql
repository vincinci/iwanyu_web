-- Live stream product purchases
-- Separate from the main orders flow because stream products are not in products table
...
-- Sellers see purchases for their sessions
CREATE POLICY "sellers_view_their_live_purchases"
  ON public.live_stream_purchases FOR SELECT
  USING (auth.uid() = seller_user_id);

-- Enable realtime for order updates
ALTER PUBLICATION supabase_realtime ADD TABLE public.live_stream_purchases;

-- ─────────────────────────────────────────────────────────────────────────────
-- RPC: purchase_live_stream_product
-- Atomically deducts wallet balance and records the purchase.
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION purchase_live_stream_product(
  p_session_id        text,
  p_product_id        text,
...
BEGIN
  ...
  -- Immediately deduct from wallet (purchase, not a bid lock)
